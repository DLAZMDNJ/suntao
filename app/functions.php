...
{
    $presenter = new \jamx\Extensions\SaasPresenter($model);
    if (empty($data)) {
...
function user($ret = 'nickname')
{
    if (Auth::check()) {
        ...
        if($ret === 'object'){
            $user = app()->make('jamx\Repositories\UserRepository');
...
if (! function_exists('fragment')) {
    ...
    {
        $content = app()->make('jamx\Repositories\ContentRepository');
...
function king_time($sec){
	$ret = $sec.'秒';
	if($sec<60){
		$ret = $sec.'秒';
	}else if($sec<3600){
		$ret = floor($sec/60).'分'.($sec%60).'秒';
	}

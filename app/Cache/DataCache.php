...
class DataCache
{
    ...
    public static function cacheCategories()
    {
        ...
        $cats = DB::table('saas_metas')
            ->select('id', 'name', 'slug')
            ->where('type', '=', 'CATEGORY')
            ->get();
        if (!empty($cats)) {
            foreach ($cats as $key=>$cat) {
                $count = DB::table('saas_contents')
    ...
    public static function cacheArchive()
    {
        ...
        if (!empty($yms)) {
            foreach ($yms as $key=>$ym) {
                $articles = DB::table('saas_contents')

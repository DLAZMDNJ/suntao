...
use jamx\Models\Setting;
use jamx\Models\SettingType;
...
class SettingRepository extends BaseRepository
{
     ...
     *
     * @param  jamx\Models\SettinType $setting_type
     * @param  jamx\Models\Setting $setting
     * @return void
     */
     ...
     * 创建或更新动态设置
     *
     * @param  jamx\Models\Setting
     * @param  array $inputs
     * @return jamx\Models\Setting
     */
     ...
     * 创建或更新动态设置分组
     *
     * @param  jamx\Models\SettingType
     * @param  array $inputs
     * @return jamx\Models\SettingType
     */
    ...
    public function index($data = [], $for = 'setting', $size = '10')
    {
        ...
        if ($for === 'setting') {
            $settings = $this->setting->join('admin_setting_type', 'admin_settings.type_id', '=', 'admin_setting_type.id')
                                     ->select('admin_settings.*', 'admin_setting_type.name as tname', 'admin_setting_type.id as tid', 'admin_setting_type.name as tname', 'admin_setting_type.value as tvalue')
                                     ->where('admin_settings.name', 'like', '%'.e($data['s_name']).'%')
                                     ->where('admin_settings.value', 'like', '%'.e($data['s_value']).'%')
                                     ->orderBy('sort', 'desc')
                                     ->orderBy('id', 'desc')
                                     ->paginate($size);
            return $settings;
     ...
     * @param  array  $inputs
     * @param  string $for 操作对象，默认对Setting操作，否则对SettingType操作
     * @return jamx\Models\Setting | jamx\Models\SettingType
     */
